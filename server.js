...
app.post("/asaas/criar-conta-teste", async (req, res) => {
  try {
    const dados = req.body;

    const ASAAS_API_KEY = "$aact_hmlg_000MzkwODA2MWY2OGM3MWRlMDU2NWM3MzJlNzZmNGZhZGY6OmUyNmFjNmUxLTIwNjAtNDU3NC05YmNiLWEwNzdmYTMzYTdmNzo6JGFhY2hfNjlhMzdjZjItZTMxMS00MWNhLTk5Y2EtNDEzMmE1ZjQ2Mjkw";
    ...
    // 🔒 Regra do Asaas (PF x PJ)
    if (dados.companyType && dados.birthDate) {
      return res.status(400).json({
        erro: "Não pode enviar companyType e birthDate juntos"
      });
    }

    if (!dados.companyType && !dados.birthDate) {
      return res.status(400).json({
        erro: "Informe companyType (PJ) ou birthDate (PF)"
      });
    }
    ...
    const response = await fetch("https://api-sandbox.asaas.com/v3/accounts", {
      ...
      body: JSON.stringify(dados) // 🔥 envia direto
    });

    const data = await response.json();

    console.log("📨 Payload recebido:", dados);
    console.log("📨 Resposta Asaas:", data);

    if (!response.ok) {
      return res.status(400).json({
        erro: "Erro ao criar conta no Asaas",
        detalhes: data
      });
    }
    ...
    res.status(500).json({
      erro: "Erro interno ao criar conta Asaas"
    });
  }
});
